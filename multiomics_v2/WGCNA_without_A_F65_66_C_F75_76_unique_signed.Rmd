...
knitr::opts_chunk$set(echo = TRUE,warning = F,message = F,fig.height=7, fig.width=7)
```
...
protein_anotation<-(data_abundance_raw)[c(4,5)]
dim(protein_anotation)
as<-bitr(protein_anotation$Accession,fromType = "UNIPROT",toType = c("SYMBOL","ENTREZID"),OrgDb = "org.Hs.eg.db",drop = T)
protein_anotation$symbol<-as$SYMBOL[match(protein_anotation$Accession,as$UNIPROT)]
protein_anotation$entrez<-as$ENTREZID[match(protein_anotation$Accession,as$UNIPROT)]
save(protein_anotation,file = "protein_anotation")
datatable_jm(data_abundance_raw[,1:20])
